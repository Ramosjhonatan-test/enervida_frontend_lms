<template>
  <div class="login-wrapper font-inter">
    ...
    <main class="relative z-20 w-full max-w-[540px] px-4 md:px-6 py-10 md:py-20 flex flex-col items-center">
      <!-- Header -->
      ...
      <div class="glass-card-premium w-full p-8 md:p-12 rounded-[48px] shadow-2xl relative overflow-hidden group/card border-accent-neon/10 animate-in fade-in slide-in-from-bottom duration-700 delay-200">
        ...
        <div class="relative z-10">
          <div class="mb-8">
            <h2 class="text-3xl md:text-4xl font-black text-on-surface mb-2 font-lexend tracking-tight">Crear Cuenta</h2>
            <div class="h-1 w-12 bg-accent-neon rounded-full mb-4"></div>
            <p class="text-on-surface-variant text-sm font-medium opacity-80">Únete a la elite de la energía renovable.</p>
          </div>
          
          <form class="space-y-6" @submit.prevent="handleRegister">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="group/input space-y-2">
                <label class="block text-[10px] font-black text-on-surface-variant uppercase tracking-[0.2em] ml-1 group-focus-within/input:text-accent-neon transition-colors">Nombres</label>
                <div class="relative">
                  ...
                  <input v-model="form.nombres" class="input-cyber pl-12" placeholder="Juan" type="text" required/>
                </div>
              </div>
              <div class="group/input space-y-2">
                <label class="block text-[10px] font-black text-on-surface-variant uppercase tracking-[0.2em] ml-1 group-focus-within/input:text-accent-neon transition-colors">Apellidos</label>
                <div class="relative">
                  <div class="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/40 group-focus-within/input:text-accent-neon transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                  </div>
                  <input v-model="form.apellidos" class="input-cyber pl-12" placeholder="Pérez" type="text" required/>
                </div>
              </div>
            </div>
            ...
            <div class="group/input space-y-2">
              ...
              <div class="relative">
                ...
                <input v-model="form.correo" class="input-cyber pl-12" placeholder="usuario@enervida.bo" type="email" required/>
              </div>
            </div>
            
            <div class="group/input space-y-2">
              <label class="block text-[10px] font-black text-on-surface-variant uppercase tracking-[0.2em] ml-1 group-focus-within/input:text-accent-neon transition-colors">Contraseña</label>
              <div class="relative">
                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/40 group-focus-within/input:text-accent-neon transition-colors">
                ...
                <input v-model="form.contrasena_hash" class="input-cyber pl-12" placeholder="••••••••" type="password" required/>
              </div>
            </div>
          ...
          <div class="flex items-center my-10 opacity-50">
            ...
            <span class="px-4 text-[9px] text-on-surface-variant font-black uppercase tracking-[0.3em]">Registro Rápido</span>
            <div class="flex-1 h-[1px] bg-on-surface/20"></div>
          </div>
          ...
          <div class="text-center mt-10">
            <p class="text-on-surface-variant text-[10px] font-black uppercase tracking-[0.2em]">
              ¿Ya eres miembro? 
              <router-link to="/login" class="text-accent-neon hover:text-on-surface ml-2 underline underline-offset-4 decoration-accent-neon/30 transition-all">Inicia Sesión</router-link>
            </p>
          </div>
        </div>
      </div>
...
const handleGoogleError = () => {
  error.value = 'Error al abrir ventana de Google o registro cancelado'
}
...
.input-cyber:focus {
    ...
    box-shadow: 0 0 20px var(--accent-neon-dim, rgba(16, 255, 157, 0.1));
}
